...
Entity 별칭과 검색 핵심어 표기 규칙은 목적이 다르므로 별도 사전으로 관리한다.

| 대상 | 적용 시점 | 적용 내용 | 금지 사항 |
|---|---|---|---|
| Entity | Fact·Entity 추출 후 | 검증된 별칭을 표준명에 연결하고 Entity ID 생성 | 검색어만으로 자동 병합 금지 |
| 핵심어 | 질문 분석 시 | 반드시 검색해야 할 개념을 안전한 표준형과 검증된 표기 별칭으로 정리 | 의미가 넓어지는 관련어 자동 추가 금지 |

Entity 별칭은 버전 관리되는 `aliases.yaml`, 핵심어 표기 규칙은 별도 `search_terms.yaml`에 저장한다. 질문 원문은 그대로 보존하고 검색 로그에는 정규화된 핵심어, 적용 규칙과 사전 버전을 기록한다. 관련어를 자동으로 추가하지 않으며 원문 Chunk, `source_span`, Fact `statement`는 표준화하지 않는다.
