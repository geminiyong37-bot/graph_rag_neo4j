...
업무추진비
  → 업무 추진비, 업무추진 비용, 업무추진비용, 업무추진경비
```

사전은 코드에 하드코딩하지 않고 버전 관리되는 JSON 또는 YAML로 관리한다. `회의비`처럼 회계 기준과 문맥에 따라 별도 계정일 수 있는 표현은 단어가 관련돼 보인다는 이유만으로 `업무추진비`에 자동 병합하지 않고 검토 대상으로 둔다.
