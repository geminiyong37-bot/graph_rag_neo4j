...
## 후속 개선 과제: 한국어 FTS 정규화

현재 `fts`는 `to_tsvector('simple', content)`로 생성하므로 한국어 형태소를 분석하지 못한다. 후속 작업에서는 아래 한계를 함께 검토한다.

- 띄어쓰기 유무: `교육용 건물`과 `교육용건물`
- 조사·어미 차이: `건물`, `건물을`, `건물의`, `매입`, `매입하다`, `매입합니다`
- 붙여쓰기와 분리 표기의 차이
- 동일 개념의 활용형 및 표기 변형

개선 방식이 결정되면 새 입력용 FTS 트리거뿐 아니라 기존 전체 행의 `fts`도 다시 생성한다. 임베딩 모델, 청크 내용 또는 청크 분할 방식이 바뀌지 않는다면 `embedding`은 다시 생성하지 않는다.
